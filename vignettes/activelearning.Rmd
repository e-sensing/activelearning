...
```{r fast_cluster_assumption, echo=FALSE, fig.width=7, fig.height=4}

svm_method <- sits::sits_svm()
...
results_lt <- list()
for (i in 1:n_iterations) {
    out_dir <- file.path(tempdir(), paste0("iter_fca_", i))
    dir.create(out_dir)
...
results_fca <- do.call(rbind, results_lt)
results_fca <- as_tibble(results_egal)
#knitr::kable(results_fca, digits = 2)

acc_plot <- results_fca %>%
    tidyr::pivot_longer(-n_samples) %>%
    ggplot2::ggplot() +
    ggplot2::geom_line(ggplot2::aes(x = n_samples,
                                    y = value,
                                    color = name)) +
    ggplot2::ggtitle("Active Learning - Fast Cluster-Assumption")
print(acc_plot)

plot(al_label_cube, main = "Fast Cluster-Assumption")
