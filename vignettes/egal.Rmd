...
oracle_samples <- tibble::tibble()
for (i in 1:n_iterations) {
  
  # Get new samples from the pool of samples.
  suppressMessages(
    egal_samples <- al_samples_tb %>%
      # NOTE: If I comment the next line (i.e. only using al_samples_tb) in all 
      #       iterations increases the accuracy!
...
accuracy_vec <- c(n_samples = nrow(al_samples_tb), accuracy_vec)

results_egal <- accuracy_vec %>%
  data.frame() %>%
  t() %>%
  magrittr::set_colnames(names(accuracy_vec)) %>%
  as_tibble()
